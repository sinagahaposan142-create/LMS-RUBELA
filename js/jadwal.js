    ...
    const body = `
      <form id="planForm" class="form">
        ...
        <div class="form-row">
          ...
          <div class="form-group"><label>Tutor Pengajar</label>
            <select name="teacherId">
              ${tutors.map(t => `<option value="${t.id}" ${editing && editing.teacherId === t.id ? 'selected' : ''}>${esc(t.name)}</option>`).join('')}
              ${tutors.length === 0 ? '<option value="">(belum ada tutor)</option>' : ''}
            </select>
          </div>
        </div>
  ...
  function renderWarSheet(container, user, state) {
    ...
    function paint() {
      ...
      const needConfirm = plans.filter(x => {
        ...
        return x.status === 'draft' && ahead >= 0 && ahead <= (settings.planConfirmLead || 5);
      });
                      ...
                      return `<tr>
                        ...
                        ${Array.from({ length: daysInMonth }, (_, i) => {
                          ...
                          return `<td class="${cls}" data-course="${c.id}" data-date="${ymd}"
                                   ${pl ? `data-plan="${pl.id}"` : ''} title="${esc(pl ? (pl.topic || '') + ' — ' + ((DB.PLAN_STATUS[pl.status] || {}).label || pl.status) : (isSched ? 'Hari mengajar — klik untuk mengisi rencana' : ''))}">
                            ${inner}
                          </td>`;
                        }).join('')}
                      </tr>`;
  ...
  function planCardHtml(pl, opts) {
    ...
    return `<div class="plan-card ${cancelled ? 'is-cancelled' : ''} ${isFixed ? 'is-fixed' : ''}">
      <div class="pc-main">
        ...
        <div class="pc-sub">${esc(lbl.sub)}${tutor ? '' : ''}</div>
        <div class="pc-meta">
          🗓️ ${UI.fmtYMD(pl.date)}
    ...
    container.innerHTML = `
      <div class="card">
        ...
        ${list.length === 0
          ...
          : `<div class="plan-list">${list.map(pl => planCardHtml(pl, { canEdit })).join('')}</div>`}
      </div>`;
